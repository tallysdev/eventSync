        ...
        <v-card>
            ...
            <v-card-text>
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-text-field v-model="newLocal.local_name" 
                        label="Nome"
                        :rules="[rules.requiredCamp && rules.required]" 
                        required></v-text-field>
                    <v-select v-model="newLocal.state" 
                        :items="UFs" 
                        item-value="value" 
                        item-title="label" 
                        label="Estado"
                    ...
                    <v-text-field
                        ...
                        required
                        @input="formatCEP"></v-text-field>
                    <v-text-field v-model="newLocal.street_name" 
                        label="Nome da Rua"
                        :rules="[rules.requiredCamp && rules.required]" 
                        required ></v-text-field>
                    <v-text-field v-model="newLocal.street_number" 
                        label="Número da Rua"
                        :rules="[rules.requiredCamp && validateNumberInput]"
                        :error-messages="errorMessages.street_number"
                        @input="validateNumberField"
                        required
                        type="number"></v-text-field>
                    <v-autocomplete v-model="newLocal.city" 
                        :items="citiesOptions" 
                        item-value="value" 
                        item-title="label"
                        label="Cidade" 
                        :rules="[rules.requiredCamp && rules.required]"></v-autocomplete>
...
import { requiredValidation, isNotEmpty, isValidCEP, UFs, validateNumberInput } from '@/utils/validation';
...
const rules = {
    ...
    validateNumberInput: (value: string) => {
        ...
        if (error) {
            errorMessages.value.street_number = [error];
            hasErrors.value = true;
        } else {
            errorMessages.value.street_number = [];
