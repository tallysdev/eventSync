      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="form" v-model="formValid">
            <v-text-field
              v-model="local.local_name"
              label="Nome"
              :rules="rules.required"
              required
            />
            <v-text-field
              v-model="local.street_name"
              label="Rua"
              :rules="rules.required"
              required
            />
            ...
            />
            <v-autocomplete v-model="local.city" 
                        :items="citiesOptions" 
                        item-value="value" 
                        item-title="label"
                        label="Cidade" 
...
const citiesOptions = ref<any[]>([]);
  ...
  const emit = defineEmits<{
    ...
    (e: 'local-updated', message: string): void;
  }>();
...
const fetchCities = async (state: string) => {
  ...
  if (local.value) {
    try {
      ...
      emit('local-updated', 'Local atualizado com sucesso')
      emit('update:dialog', false)
    } catch (error) {
      console.error('Error saving changes:', error)
      emit('local-updated', 'Erro ao atualizar o local')
    }
  }
}
