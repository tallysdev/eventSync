<template>
  ...
  <v-container fluid class="d-flex flex-column align-center">
    <v-btn color="success d-flex">Cadastrar Sala Tematica</v-btn>
    <h2 v-if="rooms.length === 0">Nenhuma Sala Cadastrada ainda...</h2>
    <v-container fluid class="d-flex flex-wrap ga-5 justify-center">
    
      <v-card class="mx-2 my-2 justify-center w-75" variant="outlined" elevation="20" max-width="400" min-width="50"
      v-for="(room, id) in rooms" :key="id">
      <v-card-actions v-if="admin" class="d-flex flex-wrap w-100 justify-space-between">
        <v-btn color="yellow" variant="text"  @click="fetchEditRoom(room.id)">Editar</v-btn>
      ...
      <v-card-actions class="d-flex flex-wrap w-100 justify-center">
        <v-btn color="blue" variant="text" @click="toggleDetails(id)">Ver Detalhes</v-btn>
...
const toggleDetails = (id: number) => {
  // Encontra a sala com o id correto
  const room = rooms.value.find((room: { id: number; }) => room.id === id)

  // Verifica se a sala foi encontrada e alterna o estado de 'showDetails'
  if (room) {
