      ...
      <v-container fluid class="pa-6 d-flex flex-column align-center">
        <v-form class="d-flex flex-column align-center">
          ...
          <div v-for="(question, index) in questions" :key="index" class="field-size">
            <v-card
              :class="['question-card', { 'invalid-card': !question.isValid }]"
              outlined
              elevation="4"
            >
              ...
              <v-card-subtitle>
                <div v-if="question.type === 'Múltipla escolha' || question.type === 'Objetiva'">
                  ...
                  <v-btn @click="addOption(index)" color="primary" class="mb-4"
                  ...
                  <div
                    ...
                    :key="optIndex"
                    class="option-container d-flex align-center mt-2"
                  >
                    <v-textarea
                      :value="option"
                      readonly
...
const questions = ref<QuestionCreate[]>([
  { text: '', type: 'Discursiva', optionList: [], isValid: true }
])
...
const submitForm = async () => {
  ...
  formData.append('event', String(1))
