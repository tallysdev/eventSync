      ...
      <v-row v-else>
        <v-col cols="12" md="6" v-for="event in events" :key="event.id">
          ...
          >
            ...
            <v-card-actions>
              <v-btn color="primary" @click.stop="editEvent(event.id)">Editar</v-btn>
              <v-btn color="error" @click.stop="confirmDelete(event.id)">Deletar</v-btn>
            </v-card-actions>
          </v-card>
        </v-col>
      </v-row>
...
const viewParticipants = (eventId: number): void => {
  router.push(`/participants/${eventId}`)
}
